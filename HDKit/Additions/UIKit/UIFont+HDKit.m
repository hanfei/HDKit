...
#import "UIFont+HDKit.h"

@implementation UIFont (HDKit)
...
+ (BOOL)loadFontFromPath:(NSString *)path {
    ...
    CFErrorRef error;
    BOOL suc = CTFontManagerRegisterFontsForURL((__bridge CFURLRef)url, kCTFontManagerScopeNone, &error);
    if (!suc) {
...
+ (BOOL)unloadFontFromPath:(NSString *)path {
    NSURL *url = [NSURL fileURLWithPath:path];
    CFErrorRef error;
    BOOL suc = CTFontManagerUnregisterFontsForURL((__bridge CFURLRef)url, kCTFontManagerScopeNone, &error);
    if (!suc) {
...
+ (UIFont *)loadFontFromData:(NSData *)data {
    ...
    CFErrorRef error;
    BOOL suc = CTFontManagerRegisterGraphicsFont(fontRef, &error);
    if (!suc) {
        CGFontRelease(fontRef);
        NSLog(@"Failed to load font from data: %@",error);
        return nil;
    }else {
        UIFont *font = [self fontWithCGFont:fontRef size:[UIFont systemFontSize]];
        CGFontRelease(fontRef);
        return font;
    }
}

+ (BOOL)unloadFont:(UIFont *)font {
    ...
    CFErrorRef error;
    BOOL suc = CTFontManagerUnregisterGraphicsFont(fontRef, &error);
    CGFontRelease(fontRef);
    if (!suc) {
        NSLog(@"Failed to unload font: %@",error);
    }
